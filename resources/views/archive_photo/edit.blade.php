...
@section('content')
    ...
    <form class="mt-3 w-100" action="{{route('archive_photo.update', $archive->id)}}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('patch')
        <div class="form-group">
            <label class="font-weight-bold" for="title">Заголовок архива альбомов *</label>
            <input type="text" class="form-control" name="title" id="title" required value="{{$archive->title}}">
        </div>

        <div class="form-group">
            <label class="font-weight-bold" for="content">Описание архива</label>
