...
@section('content')
    ...
    <form class="mt-3 w-100" action="{{route('video.store')}}" method="POST" enctype="multipart/form-data">
        @csrf
        ...
        <div class="form-group">
            ...
            <select class="form-control" name="archive_id" id="category">
                @foreach($categories as $category)
                    <option value="{{$category->id}}" {{old('archive_id') == $category->id ? 'selected' : ''}}>{{$category->title}}</option>
                @endforeach
            </select>
        </div>
        <div class="form-group">
            <label class="font-weight-bold" for="link">Ссылка на видео</label>
            <input type="text" class="form-control" name="link" id="link" required value="{{old('link')}}">
        </div>
